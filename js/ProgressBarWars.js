$.fn.extend({ProgressBarWars: function(opciones) {
        ...
        $(ProgressBarWars).append('<span class="barControl" style="width:'+opciones.tamanio+';"><div class="barContro_space" style="width: 6px; height: 100%;"><span class="'+opciones.estilo+'" style="height: '+opciones.alto+';"  id="bar'+theidProgressBarWars+'"></span></div></span>');
        $("#bar"+theidProgressBarWars).animate({height: opciones.porcentaje+"%"},opciones.tiempo);
        this.mover = function(ntamanio) {
            $("#bar"+$(this).attr("id")).animate({height:ntamanio+"%"},opciones.tiempo);
        };
        return this;
    }
});